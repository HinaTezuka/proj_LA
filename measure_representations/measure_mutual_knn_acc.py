...
# model_L2_name = "DiscoResearch/Llama3-DiscoLeo-Instruct-8B-v0.1" # german
...
dataset = load_dataset("tatoeba", lang1="en", lang2="ko", split="train")  # Replace "L2" with the actual second language
...
texts_L2 = [sample['translation']['ko'] for sample in dataset.select(range(n_samples))]
...
feats_L2 = mutual_knn_acc_funcs.extract_representations(model_L2, tokenizer_L2, texts_L2)  # Using English side for both models, change as needed
...
with open("knn_acc_ko.txt", "w") as f:
    file.write(f"Mutual KNN Accuracy: {mutual_knn_accuracy}\n")
