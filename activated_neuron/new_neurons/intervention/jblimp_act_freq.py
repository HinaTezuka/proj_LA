...
act_freq_dict = unfreeze_pickle(pkl_file_path)
# それぞれのneuronsの発火値の頻度（dict)を取得
act_freq_shared = act_freq_dict["shared_neurons"] # 現時点では非対訳ペアに発火しているshared neuronsとの重複も含む。
act_freq_L1_or_L2 = act_freq_dict["activated_neurons_L1_or_L2"]
act_freq_L2_specific = act_freq_dict["specific_neurons_L1"]
act_freq_L2_specific = act_freq_dict["specific_neurons_L2"]
...
THRESHOLD = 0
# act_freq_shared = delete_specified_keys_from_act_sum_dict(act_freq_shared, freq_base_shared, THRESHOLD)

""" shared_neurons_ONLYをpickle fileとして保存(初回のみ) """
# pkl_file_path = f"/home/s2410121/proj_LA/activated_neuron/new_neurons/pickles/same_semantics_shared_ONLY/act_freq/{active_THRESHOLD}_th/en_{L2}.pkl"
# save_as_pickle(pkl_file_path, act_freq_shared)

"""
shared_ONLY_dictをロード（同じ意味表現にのみ発火しているshared neurons: <- based on act_sum_shared）
"""
pkl_file_path = f"/home/s2410121/proj_LA/activated_neuron/new_neurons/pickles/same_semantics_shared_ONLY/act_freq/{active_THRESHOLD}_th/en_{L2}.pkl"
act_freq_shared = unfreeze_pickle(pkl_file_path)

count_shared_ONLY = 0
for layer_idx in act_freq_shared.keys():
    count_shared_ONLY += len(act_freq_shared[layer_idx])
...
for layer_idx, neurons in act_freq_shared.items():
    for neuron_idx in neurons.keys():
        shared_same_semantics.append((layer_idx, neuron_idx))
shared_same_semantics = sorted(shared_same_semantics, key=lambda x: act_freq_shared[x[0]][x[1]], reverse=True)

"""
非対訳ペアにたいして発火しているshared neurons
"""
# load act_freq_base_shared
pkl_file_path = f"/home/s2410121/proj_LA/activated_neuron/new_neurons/pickles/non_same_semantics/act_freq/{active_THRESHOLD}_th/en_{L2}.pkl"
act_freq_base_dict = unfreeze_pickle(pkl_file_path) # 非対訳ペア(2000)に発火したshared neuronsの、発火値の累計を取得
act_freq_base_shared = act_freq_base_dict["shared"]
non_translation_shared = []
for layer_idx, neurons in act_freq_base_shared.items():
    for neuron_idx in neurons.keys():
        non_translation_shared.append((layer_idx, neuron_idx))
non_translation_shared = sorted(non_translation_shared, key=lambda x: act_freq_base_shared[x[0]][x[1]], reverse=True)
...
for layer_idx, neurons in act_freq_L1_or_L2.items():
    for neuron_idx in neurons.keys():
...
# sort
layer_neuron_list_L1_or_L2 = sorted(layer_neuron_list_L1_or_L2, key=lambda x: act_freq_L1_or_L2[x[0]][x[1]], reverse=True)

""" L1のみに発火しているニューロンの中から、layer_neuron_listを作成 """
layer_neuron_list_L2_specific = []
for layer_idx, neurons in act_freq_L2_specific.items():
    for neuron_idx in neurons.keys():
...
# sort
layer_neuron_list_L2_specific = sorted(layer_neuron_list_L2_specific, key=lambda x: act_freq_L2_specific[x[0]][x[1]], reverse=True)
...
if __name__ == "__main__":
    ...
    # save as csv
    df_main.to_csv(f"/home/s2410121/proj_LA/activated_neuron/new_neurons/intervention/csv/llama3/act_freq/jblimp/{active_THRESHOLD}_th/shared_same_semantics/{dir_path}/en_{L2}.csv", index=False)
    ...
    df_shared_non_translation.to_csv(f"/home/s2410121/proj_LA/activated_neuron/new_neurons/intervention/csv/llama3/act_freq/jblimp/{active_THRESHOLD}_th/shared_non_same_semantics/{dir_path}/en_{L2}.csv", index=False)
    ...
    df_comp.to_csv(f"/home/s2410121/proj_LA/activated_neuron/new_neurons/intervention/csv/llama3/act_freq/jblimp/{active_THRESHOLD}_th/normal_COMP/{dir_path}/en_{L2}.csv", index=False)
    ...
    df_comp_L1_or_L2.to_csv(f"/home/s2410121/proj_LA/activated_neuron/new_neurons/intervention/csv/llama3/act_freq/jblimp/{active_THRESHOLD}_th/L1_or_L2/{dir_path}/en_{L2}.csv", index=False)

    """ L2 Specific """
    result_comp_L2_specific = eval_JBLiMP_with_edit_activation(model, model_name, tokenizer, layer_neuron_list_L2_specific)
    print(f"result_comp_L2_specific: {result_comp_L2_specific}")
    ...
    df_comp_L2_specific.to_csv(f"/home/s2410121/proj_LA/activated_neuron/new_neurons/intervention/csv/llama3/act_freq/jblimp/{active_THRESHOLD}_th/L2_specific/{dir_path}/en_{L2}.csv", index=False)
    ...
    print(f"overall_accuracy_comp_L2_specific: {overall_accuracy_comp_L2_specific}")
